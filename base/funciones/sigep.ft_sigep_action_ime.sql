CREATE OR REPLACE FUNCTION sigep.ft_sigep_action_ime (
...
/**************************************************************************
 ...
 FUNCION: 		sigep.ft_sigep_action_ime
 ...
 FECHA:	        17-01-2022 11:10:26
 COMENTARIOS:
***************************************************************************
 HISTORIAL DE MODIFICACIONES:
#ISSUE				FECHA				AUTOR				DESCRIPCION
 #0				17-01-2022 11:10:26								Funcion que gestiona las operaciones basicas (inserciones, modificaciones, eliminaciones de la tabla 'sigep.tsigep_adq'
 #
 ***************************************************************************/
...
BEGIN
    ...
    v_nombre_funcion = 'sigep.ft_sigep_action_ime';
    v_parametros = pxp.f_get_record(p_tabla);

    /*********************************
     #TRANSACCION:  'SIGEP_SADQ_A_INS'
     #DESCRIPCION:	Insercion de registros
     #AUTOR:		rzabala
     #FECHA:		15-03-2019 21:10:26
    ***********************************/

    if(p_transaccion='SIGEP_SADQ_A_INS')then
   ...
   ***********************************/
        ...
        /*********************************
         #TRANSACCION:  'SIGEP_COMPRODEVEN_A'
         #DESCRIPCION:	Registra la Consulta de Beneficiarios del Sigep
         #AUTOR:		rzabala
         #FECHA:		06-07-2019 15:10:26
        ***********************************/

    elsif(p_transaccion='SIGEP_COMPRODEVEN_A')then
        ...
        /*********************************
 	#TRANSACCION:  'SIGEP_UPDATE_DOCUMENTO'
 	#DESCRIPCION:	Registra la Consulta de Beneficiarios del Sigep
 	#AUTOR:		franklin.espinoza
 	#FECHA:		06-01-2022 15:10:26
	***********************************/

    elsif(p_transaccion='SIGEP_UPD_DOCUMENTO')then
            ...
            for v_record in select ten.id_entrega, ten.c31 c31_ent, ten.fecha_c31 fecha_ent, tic.id_int_comprobante
                ...
                if v_parametros.nro_preventivo != '' or v_parametros.nro_devengado != '' then
                    update conta.tint_comprobante  set
                        c31 = case
                                  when v_preve.momento = 'REG_CON_FLUJO_C21' THEN upper(concat('REG. SIP ',v_parametros.nro_devengado))
                                  when v_preve.momento = 'REG_CON_FLUJO_C21_REV' THEN upper(concat('REV. CIP ',v_parametros.nro_preventivo))
        ...
        /*********************************
         #TRANSACCION:  'SIGEP_REG_BENEF_A'
         #DESCRIPCION:	Registra la Consulta de Beneficiarios del Sigep
         #AUTOR:		rzabala
         #FECHA:		05-06-2019 16:10:26
        ***********************************/

    elsif(p_transaccion='SIGEP_REG_BENEF_A')then
    ...
    elsif(p_transaccion='SIGEP_RESULT_A')then
    ...
    elsif(p_transaccion='SIGEP_BENEF_A')then
        ...
        /*********************************
         #TRANSACCION:  'SIGEP_SADQ_A_MOD'
         #DESCRIPCION:	Modificacion de registros
         #AUTOR:		rzabala
         #FECHA:		15-03-2019 21:10:26
        ***********************************/

    elsif(p_transaccion='SIGEP_SADQ_A_MOD')then
        ...
        /*********************************
         #TRANSACCION:  'SIGEP_SADQ_A_ELI'
         #DESCRIPCION:	Eliminacion de registros
         #AUTOR:		rzabala
         #FECHA:		15-03-2019 21:10:26
        ***********************************/

    elsif(p_transaccion='SIGEP_SADQ_A_ELI')then
        ...
        /*********************************
         #TRANSACCION:  'SIGEP_RESULT_C21'
         #DESCRIPCION:	Consulta mostrar mensajes de Respuesta del Sigep
         #AUTOR:		franklin.espinoza
         #FECHA:		22-12-2021 18:00:26
        ***********************************/

    elsif(p_transaccion='SIGEP_RESULT_C21')then

        begin
            --Sentencia de la consulta
            v_valores_input = string_to_array(v_parametros.ids, ',');

            FOR i IN 1.. array_upper(v_valores_input, 1) LOOP
                v_colnames[i]:= (SELECT sig.nro_preventivo--, sig.nro_comprometido, sig.nro_devengado
                                 FROM sigep.tsigep_adq sig
                                          inner join segu.tusuario usu1 on usu1.id_usuario = sig.id_usuario_reg
                                          left join segu.tusuario usu2 on usu2.id_usuario = sig.id_usuario_mod
                ...
                v_coldev[i]:= (SELECT sig.nro_devengado
                               FROM sigep.tsigep_adq sig
                                        ...
                                        left join segu.tusuario usu2 on usu2.id_usuario = sig.id_usuario_mod
                               WHERE sig.id_sigep_adq =v_valores_input[i]::int4);
                v_matriz[i]:=concat('Doc. Devengado: ',v_colnames[i],' ||Doc. Preventivo: ', v_colcom[i],' ||Doc. Secuencia: ', v_coldev[i]);
...
ALTER FUNCTION sigep.ft_sigep_action_ime (p_administrador integer, p_id_usuario integer, p_tabla varchar, p_transaccion varchar)
    OWNER TO franklin;